...
<body>
   <div class="h-screen w-screen bg-gradient-to-r from-red-300 to-sky-300 flex flex-col gap-2 p-10 items-center">
        ...
        <div class="max-w-4xl w-full bg-white p-2 rounded-sm shadow-lg">
            @if(!isset($Item))
            <form action="/task" method="POST" enctype="multipart/form-data" class="grid grid-cols-12 gap-2">
                {{ csrf_field() }}
            ...
            @if(isset($Item))
            <form action="/task/update/{{ $Item->id }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-12 gap-2">
                {{ csrf_field() }}
                <div class="col-span-9">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="p-2 border border-slate-300 flex gap-1 justify-end">
                            @if ($Todo->done != 0)
                            <form action="/task/undone/{{ $Todo->id }}" method="POST" enctype="multipart/form-data">
                                {{ csrf_field() }}
                            ...
                            @if($Todo->done == 0)
                            <form action="/task/done/{{ $Todo->id }}" method="POST" enctype="multipart/form-data">
                                {{ csrf_field() }}
                            ...
                            <form action="/task/edit/{{ $Todo->id }}" method="POST" enctype="multipart/form-data">
                                {{ csrf_field() }}
                            ...
                            <form action="/task/destroy/{{ $Todo->id }}" method="get" enctype="multipart/form-data">
                                {{ csrf_field() }}
                    ...
                    <tr>
                        ...
                        <td class="p-2 border border-slate-300 flex gap-1 justify-end">
                            ...
                            <form action="/task/destroy/{{ $Todo->id }}" method="POST" enctype="multipart/form-data">
                                {{ csrf_field() }}
                                <input type="submit" name="submit" value="Delete" class="px-2 bg-red-400 text-white rounded-md hover:bg-red-600 font-semibold hover:cursor-pointer">
                            </form>
                        </td>
